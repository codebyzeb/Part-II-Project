...
\usepackage{graphicx, caption}  % allows inclusion of PDF, PNG and JPG images
\usepackage{verbatim}
...
\texttt{texcount -subcount=chapter -merge -brief -sum diss.tex}
}
\footnotetext[2]{This line count was computed using: \texttt{cat *.py | wc -l}
}
...
Language has evolved and therefore probably gave an evolutionary advantage to the individuals that exhibited it. According to \citet{Cangelosi1998}, it is difficult to investigate the evolutionary origin of language and the selective pressures that may have originated language due to the limited evidence available. In the referenced paper, they describe using a simulated world to investigate the effect of introducing language to a population of agents controlled by neural networks. These agents interact with an environment of mushrooms that are edible and poisonous and must correctly categorise them to survive. Exploring this simulation is the basis of my project.
